...
const SearchInput: React.FC<SearchInputProps> = ({
  ...
  return (
    <div className="relative w-full group" ref={containerRef}>
      ...
      <div className="relative flex items-center gap-2 w-full rounded-xl bg-[#0a0a0f]/80 backdrop-blur-md border border-theme-500/30 group-hover:border-theme-500/60 group-focus-within:border-theme-500 shadow-[inset_0_0_15px_rgba(var(--theme-500),0.05)] group-focus-within:shadow-[0_0_20px_rgba(var(--theme-500),0.2),inset_0_0_10px_rgba(var(--theme-500),0.1)] transition-all duration-300 px-4 py-3 cyber-glitch">
        {/* HUD装飾 */}
        <div className="absolute top-0 right-0 w-4 h-4 border-t border-r border-theme-500/40 group-focus-within:border-theme-500 pointer-events-none rounded-tr-lg" />
        <div className="absolute bottom-0 left-0 w-4 h-4 border-b border-l border-theme-500/40 group-focus-within:border-theme-500 pointer-events-none rounded-bl-lg" />
        ...
        {value && (
          <button
            onClick={() => setValue("")}
            className="p-1.5 rounded-lg hover:bg-theme-500/20 transition-colors border border-transparent hover:border-theme-500/30"
            aria-label="検索をクリア"
          >
            <IoMdClose
      ...
      {/* 検索履歴ドロップダウン */}
      {showHistory && (
        <div className="absolute top-full left-0 right-0 mt-2 rounded-xl bg-[#0a0a0f]/95 backdrop-blur-xl border border-theme-500/40 shadow-[0_10px_40px_rgba(0,0,0,0.8),0_0_20px_rgba(var(--theme-500),0.1)] z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <ul className="max-h-64 overflow-y-auto custom-scrollbar p-1">
            {history.map((query) => (
              <li
                key={query}
                className="flex items-center w-full hover:bg-theme-500/10 transition-colors group/item rounded-lg m-1"
              >
                <button
                  onClick={() => handleHistoryClick(query)}
                  className="flex-1 flex items-center gap-3 px-3 py-2 text-left text-sm text-theme-300 font-mono hover:text-white min-w-0"
                >
                  <BiSearch
                    className="text-theme-500/60 flex-shrink-0"
                    size={16}
                  />
                  <span className="truncate uppercase tracking-tight">{query}</span>
                </button>
                <button
                  onClick={(e) => handleRemoveQuery(e, query)}
                  className="mr-2 p-1 rounded-md opacity-0 group-hover/item:opacity-100 hover:bg-theme-500/30 text-theme-400 hover:text-white transition-all flex-shrink-0 border border-transparent hover:border-theme-500/40"
                  aria-label="履歴を削除"
                >
                  <IoMdClose size={14} />
                </button>
