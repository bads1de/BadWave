...
const UserCard: React.FC<UserCardProps> = ({ userDetails, isCollapsed }) => {
  ...
  if (!userDetails) {
    ...
    return (
      <Card className="mx-2 overflow-hidden rounded-xl bg-gradient-to-br from-neutral-800/80 to-neutral-900/80 backdrop-blur-lg border-white/5 hover:border-purple-500/30 transition-all duration-500 shadow-lg hover:shadow-purple-500/10">
        <button
          onClick={authModal.onOpen}
          className="w-full p-3 relative hover:bg-purple-500/10 transition-all duration-300"
        >
        ...
        <div className="p-3 relative">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => router.push("/account")}
                className="p-1.5 rounded-xl hover:bg-purple-500/10 transition-colors duration-300"
              >
                <Settings
                  size={16}
