      ...
      >
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            <p className="text-center text-theme-500 animate-pulse font-mono tracking-[0.5em] text-xl">
              [ ANALYZING_TREND_STREAM... ]
            </p>
            <div className="w-64 h-1 bg-theme-900 mt-4 overflow-hidden relative border border-theme-500/20">
               <div className="absolute top-0 left-0 h-full bg-theme-500 animate-slide-in w-1/3 shadow-[0_0_10px_rgba(var(--theme-500),0.8)]" />
            </div>
          </div>
        ) : error ? (
          <p className="text-red-500 text-center font-mono uppercase tracking-widest">[ ERROR: {error.message} ]</p>
        ) : (
          <ScrollableContainer showArrows={showArrows}>
            {trends?.map((song, index) => (
              ...
              >
                <div className="relative w-full h-64 overflow-hidden rounded-xl bg-[#0a0a0f] border border-theme-500/20 group-hover:border-theme-500/60 shadow-[0_0_15px_rgba(var(--theme-500),0.1)] group-hover:shadow-[0_0_30px_rgba(var(--theme-500),0.3)]">
                  {/* HUDコーナー */}
                  ...
                  <Image
                    ...
                    fill
                    unoptimized
                    className="rounded-t-xl group-hover:scale-125 group-hover:opacity-60 transition-all duration-700 ease-out"
                  ...
                  <div className="absolute top-4 left-4 z-20">
                    <div className="bg-theme-500/20 backdrop-blur-md border border-theme-500/40 px-3 py-1 rounded-none font-mono text-xl text-white shadow-[0_0_10px_rgba(var(--theme-500),0.4)]">
                      <span className="text-theme-400">#</span>{index + 1}
                    </div>
                  </div>
                ...
                <div className="p-4 font-mono uppercase">
                  ...
                  <div className="text-theme-500 text-xs tracking-widest">
                    <ScrollingText text={`// AUTH: ${song.author}`} limitCharacters={22} />
                  </div>
